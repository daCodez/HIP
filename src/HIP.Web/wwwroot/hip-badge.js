(function renderHipTrustBadges() {
  ...
  const STYLE_ID = "hip-trust-badge-style-v2";
  ...
  async function togglePanel(container, button, panel, badge, apiBase) {
    ...
    try {
      ...
      container.className = container.className.replace(/hip-state-[a-z-]+/, "hip-state-unable-to-verify");
      container.querySelector(".hip-badge-score").textContent = "—";
      panel.innerHTML = `
        <header class="hip-panel-header"><div><small>HIP trust status</small><strong>Unable to verify</strong></div><button type="button" class="hip-panel-close" aria-label="Close trust details">×</button></header>
        <p class="hip-panel-warning">HIP could not independently revalidate this certificate. Do not rely on the visual badge.</p>
  ...
  function shieldMarkup() {
    return `<svg viewBox="0 0 48 56" role="img" aria-label=""><defs><linearGradient id="hip-shield-gradient" x1="0" y1="0" x2="1" y2="1"><stop stop-color="#06b6d4"/><stop offset="1" stop-color="#3082F6"/></linearGradient></defs><path d="M24 2 44 10v15c0 14-8 23-20 29C12 48 4 39 4 25V10L24 2Z" fill="none" stroke="url(#hip-shield-gradient)" stroke-width="4"/><path d="m15 27 6 6 13-15" fill="none" stroke="url(#hip-shield-gradient)" stroke-width="4" stroke-linecap="round" stroke-linejoin="round"/></svg>`;
  }
