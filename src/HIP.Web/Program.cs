...
public partial class Program
{
    ...
    /// <summary>
    /// Produces a domain-bound loader for the shared badge runtime. The host page may choose
    /// presentation attributes, but it cannot supply score, identity, or certificate state.
    /// </summary>
    public static string BuildBadgeScript(string domain)
    {
        var domainLiteral = JsonSerializer.Serialize(domain);
        return $$"""
(function loadHipLiveTrustBadge() {
  "use strict";
  ...
  const selector = `[data-hip-badge="${CSS.escape(domain)}"], .hip-trust-badge[data-domain="${CSS.escape(domain)}"]`;
  let container = document.querySelector(selector);
  if (!container) {
    container = document.createElement("div");
    container.setAttribute("data-hip-badge", domain);
    (currentScript && currentScript.parentNode ? currentScript.parentNode : document.body).insertBefore(container, currentScript || null);
  }
  container.classList.add("hip-trust-badge");
  ...
  runtime.onerror = () => {
    container.textContent = "HIP unable to verify";
    container.setAttribute("data-hip-runtime", "unavailable");
  };
