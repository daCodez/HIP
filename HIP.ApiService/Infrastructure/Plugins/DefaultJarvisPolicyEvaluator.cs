...
/// <summary>
/// Default policy evaluator implementation (candidate for policy-pack replacement).
/// </summary>
public sealed class DefaultJarvisPolicyEvaluator(
    ...
    ILogger<DefaultJarvisPolicyEvaluator> logger) : IJarvisPolicyEvaluator
{
    ...
    /// <inheritdoc />
    public async Task<JarvisPolicyEvaluationResultDto> EvaluateAsync(JarvisPolicyEvaluationRequestDto request, CancellationToken cancellationToken)
    {
        ...
        var sanitizedText = decision == "block" ? string.Empty : string.Join('\n', sanitizedLines).Trim();
