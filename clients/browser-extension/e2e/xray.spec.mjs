...
test("X-ray is explicit, interactive, privacy-safe, and fully removable", async () => {
  ...
  try {
    await runtime.context.route("http://localhost:4180/**", route => route.fulfill({
      ...
      body: route.request().url().endsWith(".js") ? "globalThis.lateScriptLoaded = true;" : `<!doctype html>
    ...
    await expect(page.getByRole("heading", { name: "X-ray findings" })).toHaveCount(0);
    ...
    await expect(page.getByRole("heading", { name: "X-ray this page" })).toBeVisible();
    ...
    await expect(page.getByText("hip · trust result", { exact: true })).toBeVisible({ timeout: 5_000 });
    ...
    await expect(page.getByRole("button", { name: /Show finding .*Password form is not fully encrypted/ })).toBeVisible();
    ...
    await page.getByRole("button", { name: /Show finding .*Password form is not fully encrypted/ }).click();
