...
public static class AuditEndpoints
{
    ...
    {
        async Task<IResult> getAuditEventsHandler(
        ...
        endpoints.MapGet("/api/admin/audit", getAuditEventsHandler)
        ...
        endpoints.MapGet("/api/v1/admin/audit", getAuditEventsHandler)
            .RequireRateLimiting("read-api")
            .WithName("GetAuditEventsV1")
            .WithSummary("Query admin audit events")
            .WithDescription("Versioned alias of admin audit query endpoint.")
            .WithTags("Admin", "v1")
            .Produces(StatusCodes.Status200OK)
            .Produces(StatusCodes.Status429TooManyRequests);

        async Task<IResult> exportAuditEventsHandler(
            HttpContext httpContext,
            int? take,
            string? format,
            string? eventType,
        ...
        endpoints.MapGet("/api/admin/audit/export", exportAuditEventsHandler)
            .RequireRateLimiting("read-api")
        ...
        endpoints.MapGet("/api/v1/admin/audit/export", exportAuditEventsHandler)
            .RequireRateLimiting("read-api")
            .WithName("ExportAuditEventsV1")
            .WithSummary("Export admin audit events")
            .WithDescription("Versioned alias of admin audit export endpoint.")
            .WithTags("Admin", "v1")
            .Produces(StatusCodes.Status200OK)
            .Produces(StatusCodes.Status429TooManyRequests);
