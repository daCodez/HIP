...
public sealed class CryptoStartupValidationTests
{
    ...
    [Test]
    public void Startup_WithInvalidPolicyThresholdOrder_ThrowsInvalidOperationException()
    {
        using var app = new WebApplicationFactory<Program>()
            .WithWebHostBuilder(builder =>
            {
                builder.UseEnvironment("Development");
                builder.UseSetting("HIP:Crypto:Provider", "Placeholder");
                builder.UseSetting("HIP:Policy:LowRiskRequiredScore", "70");
                builder.UseSetting("HIP:Policy:MediumRiskRequiredScore", "50");
                builder.UseSetting("HIP:Policy:HighRiskRequiredScore", "90");
            });
        ...
        Assert.That(ex, Is.Not.Null);
        Assert.That(ex!.Message, Does.Contain("low <= medium <= high"));
    }

    [Test]
    public void Startup_WithEmptyPolicyVersion_ThrowsInvalidOperationException()
    {
        using var app = new WebApplicationFactory<Program>()
            .WithWebHostBuilder(builder =>
            {
                builder.UseEnvironment("Development");
                builder.UseSetting("HIP:Crypto:Provider", "Placeholder");
                builder.UseSetting("HIP:Policy:Version", " ");
            });
        ...
        Assert.That(ex, Is.Not.Null);
        Assert.That(ex!.Message, Does.Contain("HIP:Policy:Version must not be empty"));
    }
}
