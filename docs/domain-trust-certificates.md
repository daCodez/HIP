...
The schema carries algorithm identifiers and supports provider selection, but the current repository implementation is **not quantum-resistant** and does not claim ML-DSA-65 support. An audited provider, managed key custody integration, interoperability vectors, and deployment evidence are required before any post-quantum claim.
