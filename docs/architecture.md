...
Private signing keys remain behind `IManagedTrustReceiptSigner` and never enter application contracts or persistence. The secure default implementation is unavailable. Deployment composition may explicitly select the software-backed SoftHSM PKCS #11 starter provider, while an audited managed HSM or remote signer can replace it at the same boundary. Every provider requires authoritative public key lifecycle state and an explicit authority/key allowlist. See [ADR-013](decisions/ADR-013-domain-certificate-and-badge-trust-model.md) and [HIP Domain Trust Certificates](domain-trust-certificates.md).
