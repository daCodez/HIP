...
public sealed class AdminAuthoritativeDnsPageTests
{
    ...
    public void Page_and_navigation_use_owner_only_authorization_and_real_service_state()
    {
        ...
        Assert.Multiple(() =>
        {
            ...
            Assert.That(page, Does.Contain("Confirm disable"));
            Assert.That(page, Does.Contain("DnsManagement.DisableAsync("));
            Assert.That(page, Does.Contain("_pendingDisableDomain != domain"));
